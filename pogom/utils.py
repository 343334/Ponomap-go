...
def get_args():
    ...
    parser.add_argument('-wph', '--workers_per_hive',
                        help='Workers in each hex scan',
                        type=int, default=1)
    parser.add_argument('-bvc', '--by_valid_count',
                        help='Increase scan area until X hives with scannable items have been found', type=int)
    parser.add_argument('-bhc', '--by_hex_count',
                        help='Increase scan area until X hives have been calculated', type=int)
    parser.add_argument('-lp', '--leaps', help='Beehive rings', type=int)
    ...
    parser.add_argument('-st', '--step-limit', help='Steps.', type=int,
                        default=12)
    parser.add_argument('-std', '--step_distance', help='Distance between scans', type=int,
                        default=70)
    ...
    parser.add_argument('-lr', '--login-retries',
                        ...
                        type=int, default=3)
    parser.add_argument('-sr', '--scan_retries',
                        help='Number of scan attempts before moving on.',
                        type=int, default=3)
    parser.add_argument('-ft', '--focus_tth',
                        help='Focus on finding valid despawn times.',
                        action='store_true', default=False)
    ...
    parser.add_argument('-nk', '--no-pokestops',
                        ...
                        action='store_true', default=False)
    parser.add_argument('-us', '--usestops',
                        help='Use with --skip-empty: use known pokestops and known spawnpoints for scanning.',
                        action='store_true', default=False)
    parser.add_argument('-igl', '--ignore-list',
                        help='Ignores Pokemon from the map (including parsing them into local db)',
                        action='append', default=[])
    parser.add_argument('-sp', '--spawn_delay', type=int,
                        help='Seconds to wait before scanning spawnpoint.', default=10)
    ...
    parser.add_argument('-clss', '--sscluster',
                        help='Cluster spawnpoints before scanning (with -ss and no .json)', action='store_true', default=False)
    parser.add_argument('-clt', '--cluster_time',
                        help='Time threshold for spawnpoint clustering', type=int, default=0)
    parser.add_argument('-spw', '--spawns_per_worker', type=int,
                        help='Max spawnpoints for a worker to handle in a single hex area.', default=10000)
    ...
    if not args.only_server:
        if args.leaps > 1 and not args.beehive:
            print("Cannot use leaps without beehive")
            time.sleep(5)
            sys.exit(1)
    ...
    if args.only_server:
        ...
        if args.status_name is None:
            print("You need to specify a status-name so workers and spawnpoints can be assigned properly. This will also make it easier to identify your process screens.")
            time.sleep(3)
            sys.exit(1)
        if args.beehive is False and args.step_limit > 4 and args.no_pokemon is False and args.spawnpoint_scanning is False:
            print("Detected large step size while searching for pokemon. Recommended to use -bh with anything requiring more than -st 4 (we're not speedscan). Continuing in 5 seconds...")
            time.sleep(5)
...
def hoursecs(time=now()):
    return time - (time % 3600)  # current hour, without the seconds
