...
function gopaywall_loaded() {
  username = userobj.user_data.username
}
...
function reportGhost (encounter_id) { // eslint-disable-line no-unused-vars
  mapData.pokemons[encounter_id].marker.setMap(null)
  mapData.pokemons[encounter_id].hidden = true
  ghostId.push(encounter_id)
}
...
function pokemonLabel (name, rarity, types, disappearTime, id, latitude, longitude, encounterId, atk, def, sta, move1, move2, valid, weight, height, gender, form, last_modified, spawnpoint_id, ghost) {
  ...
  if (valid == 1) {
	if (ghost > 0) {
		var dspawn = `Reported Ghost (${ghost}) `
	} else {
		var dspawn = `Verified timer `
	}
    dspawn += `<a href='javascript:reportGhost("${encounterId}")'>Report</a></b>`
  } else {
    var dspawn = `Unverified timer`
  }
  ...
  var contentstring = `
    <div>
      <a href='https://pokemongo.gamepress.gg/pokemon/${id}' target='_blank' title='View in Pokedex'>
	<b>${name}</b>
      <span> - </span>
      <small>
        #${id}</a>
    ...
    <div>` + dspawn + ` | Found at ${pad(foundDate.getHours())}:${pad(foundDate.getMinutes())}:${pad(foundDate.getSeconds())}</div><div>
    ...
    <div>
	  ...
	  <a href='javascript:void(0);' onclick='javascript:openMapDirections(${latitude},${longitude});' title='View in Maps'>SID: ${spawnpoint_id}</a>
    </div>`
...
function gymLabel(item) {
  ...
  for (var i = 0; i < members.length; i++) {
    memberStr += `
      <span class="gym-member" title="${members[i].pokemon_name}">
  ...
  var nameStr = (name ? `<div><a href='javascript:void(0);' onclick='javascript:openMapDirections(${latitude},${longitude});' title='View in Maps'>${name}</a></div>` : '')
...
function getGymMarkerIconURL(item) {
    ...
    if (item['raid_pokemon_id'] != null) {
        ...
        return 'static/forts/shield/' + gymTypes[item['team_id']] + '_' + item['raid_level'] + '_raid_' + item['raid_pokemon_id'] + '.png'
    } else if (item['raid_level'] != null) {
        var raidEgg = item['raid_level'] <= 2 ? 'normal' : 'rare'
        console.log("Got RAID SPAWNED with Egg " + raidEgg)
        return 'static/forts/shield/' + gymTypes[item['team_id']] + '_' + item['raid_level'] + '_raid_' + raidEgg + '.png'
    } else {
...
function customizePokemonMarker (marker, item, skipNotification) {
  ...
  if (notifiedPokemon.indexOf(item['pokemon_id']) > -1 || notifiedRarity.indexOf(item['pokemon_rarity']) > -1) {
    if (!skipNotification) {
      ...
      sendNotification('A wild ' + item['pokemon_name'] + ' appeared!', 'Click to load map', 'static/highres_icons/' + item['pokemon_id'] + '.png', item['latitude'], item['longitude'])
    }
  ...
  if (item['individual_attack'] != null) {
    ...
    if (notifiedMinPerfection > 0 && perfection >= notifiedMinPerfection) {
      if (!skipNotification) {
        ...
        sendNotification('A ' + perfection.toFixed(1) + '% perfect ' + item['pokemon_name'] + ' appeared!', 'Click to load map', 'static/highres_icons/' + item['pokemon_id'] + '.png', item['latitude'], item['longitude'])
      }
      if (marker.animationDisabled !== true) {
        marker.setAnimation(google.maps.Animation.BOUNCE)
      }
...
function clearStaleMarkers () {
  $.each(mapData.pokemons, function (key, value) {
    if (mapData.pokemons[key]['disappear_time'] < new Date().getTime() ||
        ...
        excludedType.indexOf(mapData.pokemons[key]['pokemon_types']) >= 0) {
  ...
  $.each(mapData.lurePokemons, function (key, value) {
    if (mapData.lurePokemons[key]['lure_expiration'] < new Date().getTime() ||
        ...
        excludedType.indexOf(mapData.lurePokemons[key]['pokemon_types']) >= 0) {
...
function processPokemons (i, item) {
  ...
  if (!(item['encounter_id'] in mapData.pokemons) &&
      excludedPokemon.indexOf(item['pokemon_id']) < 0 && excludedPokemon.indexOf(item['pokemon_rarity']) < 0 && excludedPokemon.indexOf(item['pokemon_types']) < 0 && item['disappear_time'] > Date.now()) {
...
function processGyms (i, item) {
  ...
  var openslots = item.slots_available
  ...
  var gymHasOpenSpot = function (openslots, pokemonInGym) {
      return openslots > 0 && item.pokemon.length !== 0
  }

  if (Store.get('showOpenGymsOnly') === 1) {
      if (!gymHasOpenSpot(openslots, item.pokemon.length)) {
  ...
  if (item['raid_level'] < Store.get('minGymLevel')) {
      removeGymFromMap(item['gym_id'])
      return true
  }
...
function showGymDetails (id) { // eslint-disable-line no-unused-vars
  ...
  data.done(function (result) {
    if (result.raid_level < 1) {
	  var gymLevel = 6 - result.slots_available
    } else {
        ...
        pokemonHtml += `
          <tr onclick=toggleGymPokemonDetails(this)>
            ...
            <td width="190" class="team-${result.team_id}-text" align="center">
              ...
              <div style="line-height: 1em;">Trainer not available</div>
            </td>
            <td width="10">
...
$(function () {
  ...
  $selectTypeExcluded = $('#excluded-type')
  ...
  $.getJSON('static/dist/data/pokemon.min.json').done(function (data) {
	...
	$selectTypeExcluded.select2({
      placeholder: i8ln('Select Type'),
      data: [i8ln('Normal'), i8ln('Fighting'), i8ln('Flying'), i8ln('Poison'), i8ln('Ground'), i8ln('Rock'), i8ln('Bug'), i8ln('Ghost'), i8ln('Steel'), i8ln('Fire'), i8ln('Water'), i8ln('Grass'), i8ln('Electric'), i8ln('Psychic'), i8ln('Ice'), i8ln('Dragon'), i8ln('Fairy'), i8ln('Dark')],
      templateResult: formatState
    })
	...
	$selectTypeExcluded.on('change', function (e) {
      excludedType = $selectTypeExcluded.val().map(String)
      Store.set('remember_select_type_excluded', excludedType)
    })
	...
	$selectTypeExcluded.val(Store.get('remember_select_type_excluded')).trigger('change')
